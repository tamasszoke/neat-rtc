...
const NeatRTC = (config, sendCallback) => {
  ...
  */
  const datachannel = (type, channelName, callbacks, binaryType) => {
    if (type.toLowerCase() === 'add') {
      let channel = peerConnection['data'].createDataChannel(channelName, { reliable: true })
  ...
  */
  const createOffer = (connectionType, SDP_CONSTRAINTS) => {
    peerConnection[connectionType].createOffer((description) => {
  ...
  */
  const createAnswer = (connectionType, SDP_CONSTRAINTS) => {
    peerConnection[connectionType].createAnswer((description) => {
      peerConnection[connectionType].setLocalDescription(description)
  ...
  */
  const onIceCandidate = (connectionType) => {
    peerConnection[connectionType].onicecandidate = (e) => {
  ...
  */
  const onDatachannel = (connectionType) => {
    peerConnection[connectionType].ondatachannel = (e) => {
  ...
  */
  const onStreamAdd = (connectionType) => {
    peerConnection[connectionType].onaddstream = (e) => {
  ...
  */
  const onDefaultDatachannelOpen = (channel) => {
    peerConnection['data'].datachannelList[channel].onopen = (e) => {
  ...
  */
  const onDefaultDatachannelMessage = (channel) => {
    peerConnection['data'].datachannelList[channel].onmessage = (e) => {
  ...
  */
  const onDefaultDatachannelError = (channel) => {
    peerConnection['data'].datachannelList[channel].onerror = (e) => {
  ...
  */
  const onDefaultDatachannelClose = (channel) => {
    peerConnection['data'].datachannelList[channel].onclose = (e) => {
  ...
  */
  const checkConnection = (channel) => {
    if (peerConnection['data']) {
      ...
      return channelData.readyState === 'open'
    } else {
      return false
    }
  }
